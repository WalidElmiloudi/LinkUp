                        ...
                        <form method="POST" action="{{ route('profile.update') }}" enctype="multipart/form-data">
                            ...
                            <div class="bg-white rounded-xl shadow-sm overflow-hidden">
                                ...
                                <div class="p-6">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="text-sm text-gray-500 text-center">Taille maximale : 5MB. Formats acceptés
                                            : JPG, PNG, GIF.</p>
                                    </div>
                                </div>
                            </div>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div class="md:col-span-2">
                                    <label for="username" class="block text-sm font-medium text-gray-700 mb-1">Pseudo
                                        unique</label>
                                    <input type="text" name="username"
                                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 focus:outline-none transition-colors"
                                        value="{{ auth()->user()->username }}" required>
                                    <p class="mt-1 text-sm text-gray-500">Ce pseudo sera visible par les autres membres et
                                        doit être unique.</p>
                                </div>
                                <div class="md:col-span-2">
                                    ...
                                    <div class="flex items-center">
                                        <input type="email" name="email"
                                            class="flex-1 px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 focus:outline-none transition-colors"
                                            value="{{ auth()->user()->email }}" required>
                                        <span
                                            class="ml-3 inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800">
                                            <i class="fas fa-check-circle mr-1"></i> Vérifié
                                        </span>
                                    </div>
                                    <p class="mt-1 text-sm text-gray-500">Un email de vérification sera envoyé si vous
                                        changez votre adresse.</p>
                                </div>
                            </div>

                            <div class="mt-8 pt-6 border-t border-gray-200 flex justify-end space-x-4">
                                <button type="button" data-page="profile"
                                ...
                                <button type="submit"
                                    class="px-6 py-3 border border-transparent rounded-lg font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors">
